...
function useOtherNow(otherKey) {
  // 전역 스토어에서 바로 읽음 (localStorage polling 불필요)
  const { marketData } = useAppStore();
  const otherCode = otherKey === "dashboard" ? "418660" : "465610";
...
export default function Stock2Page() {
  // [수정] dailyCache, setDailyCache 사용
  ...
  // [수정] 전역 데이터 사용 (GlobalMarketWatcher 연동)
  ...
  // [수정] 캐시에 데이터 있으면 그것을 초기값으로 사용 -> 로딩 없음
  ...
  useEffect(() => {
    // [핵심] 캐시에 데이터가 있으면 API 호출 건너뜀 (멈춤 현상 해결)
    ...
    (async () => {
      try {
        ...
        setApiRows(resRows);
        setDailyCache(CODE, resRows); // [핵심] 로딩한 데이터 캐시에 저장
      } catch (e) {
        console.error(e);
      }
    })();
  }, [dailyCache, setDailyCache]); // 의존성에 dailyCache 추가
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto" }}>
        ...
        <section style={cardWrap}>
          {(() => {
            ...
            // otherNow use hook logic embedded above
