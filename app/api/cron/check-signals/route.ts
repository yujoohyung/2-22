...
// 크론이 내부 API를 호출할 절대주소. 배포 URL 또는 로컬
const BASE =
...
function rsiCutler(values: number[], period = 14) {
  const n = values.length,
    out = Array(n).fill(null) as (number | null)[];
...
async function fetchDaily(code: string) {
  ...
  const start = `${s.getFullYear()}${pad(s.getMonth() + 1)}${pad(
    s.getDate()
  )}`;
  const res = await fetch(
    `${BASE}/api/kis/daily?code=${code}&start=${start}&end=${end}`,
    { cache: "no-store" }
  );
...
async function fetchNow(code: string) {
  const res = await fetch(`${BASE}/api/kis/now?code=${code}`, {
    cache: "no-store",
  });
...
export async function GET() {
  try {
    // ❗️서비스 롤로만 접근 (RLS 우회) — 절대 supaClient(브라우저용) 사용 금지
    const supa = getServiceClient();

    // 1) 알림 대상 사용자 로드
    //    필요 시 v_user_settings_allowed 뷰로 바꿔도 됨.
    const { data: users, error } = await supa
      .from("user_settings")
      .select("user_id, user_email, notify_enabled, stage_amounts_krw, telegram_chat_id")
      .eq("notify_enabled", true);
    if (error) throw error;
    if (!users?.length)
      return NextResponse.json({ ok: true, msg: "no users" });
    ...
    const lastIdx = closeN.length - 1;
    const lastRSI = (rsiN[lastIdx] ?? null) as number | null;
    const below200 =
      ma200N[lastIdx] != null && closeN[lastIdx] < (ma200N[lastIdx] as number);
    ...
    for (const s of users as any[]) {
      const levels: number[] = Array.isArray(s.rsi_buy_levels)
        ? s.rsi_buy_levels
        : [43, 36, 30];
      const stageAmounts: number[] = Array.isArray(s.stage_amounts_krw)
        ? s.stage_amounts_krw
        : [0, 0, 0];
      const stg = stageFromRSI(lastRSI, levels);

      if (!stg && !below200) {
        results.push({ email: s.user_email, skipped: true });
        continue;
      }

      // 최근 60분 중복 차단 (레벨 기준)
      const levelText = stg ? `${stg}단계` : "리밸런싱";
      const { data: recent } = await supa
        ...
        .limit(1);
      if (recent?.length) {
        results.push({ email: s.user_email, dedup: true });
        continue;
      }
      ...
      if (stg) {
        ...
        const qtyN =
          nowN.price > 0 ? Math.max(1, Math.floor(amtN / nowN.price)) : 0;
        const qtyB =
          nowB.price > 0 ? Math.max(1, Math.floor(amtB / nowB.price)) : 0;
      ...
      if (!stg && below200) {
        ...
        if (!sentThisYear?.length) {
          ...
          if (s.telegram_chat_id) await sendTelegram(s.telegram_chat_id, t);
        }
      }
