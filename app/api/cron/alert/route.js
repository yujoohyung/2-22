...
const fmtKST = (d) =>
  `${d.getFullYear()}-${to2(d.getMonth() + 1)}-${to2(d.getDate())} ${to2(d.getHours())}시 ${to2(d.getMinutes())}분`;
...
// 단계 판정: [43,36,30] → 0/1/2 (표시로는 1/2/3단계), 미충족 -1
...
export async function GET(req) {
  try {
    ...
    const force = url.searchParams.get("force") === "1"; // 강제 실행
