      ...
      try {
        ...
        if (data?.session) {
          ...
          if (d?.data?.yearly_budget) {
            const v = Number(d.data.yearly_budget);
            setYearlyBudget(v); setInputBudget(v);
          }
        }
      } catch (e) {
    ...
    <div className="cash-container">
      ...
      <div className="input-card">
        ...
        <div className="input-row">
          <input type="number" value={inputBudget} onChange={(e) => setInputBudget(Number(e.target.value))} placeholder="금액 입력" />
          <button onClick={handleSave}>저장</button>
        </div>
        <div className="info-row">
          <div><span className="label">나스닥(60%) 월평균:</span> <span className="val">{won(monthNasdaq)}</span></div>
          <div><span className="label">빅테크(40%) 월평균:</span> <span className="val">{won(monthBigTech)}</span></div>
        </div>
      </div>
      <div className="table-card">
        <div className="header-row">
          <div className="th">구분</div>
          <div className="th">나스닥 2배 <br/><span className="price-tag">{priceN > 0 ? won(priceN) : "(대기중...)"}</span></div>
          <div className="th">빅테크 2배 <br/><span className="price-tag">{priceB > 0 ? won(priceB) : "(대기중...)"}</span></div>
        </div>
  ...
  return (
    <>
      <div className="row">
        ...
        <div className="val-col"><div className="qty">{q1}주</div><div className="amt">({won(a1)})</div></div>
        <div className="val-col"><div className="qty">{q2}주</div><div className="amt">({won(a2)})</div></div>
      </div>
      <style jsx>{`
