  ...
  useEffect(() => {
    const fetchDaily = async () => {
      try {
        ...
        const s = new Date(d); s.setDate(s.getDate() - 400);
...
export default function TotalPage() {
  ...
  const factor = 0.92;
  let stageRatio = 0;
  if (stage === 1) stageRatio = 0.14; else if (stage === 2) stageRatio = 0.26; else if (stage === 3) stageRatio = 0.60;
  ...
  let displayN = "-", displayB = "-";
  if (nasdaq.ready) {
    if (signalType === "SELL") { displayN = "30% 매도"; displayB = "30% 매도"; }
    else if (signalType === "BUY") { displayN = `${qtyN}주 매수`; displayB = `${qtyB}주 매수`; }
    else { displayN = "관망"; displayB = "관망"; }
  }
    ...
    <div className="total-wrap">
      ...
      <div className="status-table">
        <div className="t-row">
          ...
          <div className="cell val">{priceN > 0 ? won(priceN) : "..."}</div>
        </div>
        <div className="t-row">
          ...
          <div className="cell label">현재가</div>
          <div className="cell val">{priceB > 0 ? won(priceB) : "..."}</div>
        </div>
        ...
        <div className="action-row">
          <div className="action-col">
            ...
            <div className="act-val" style={{ color: signalType === "HOLD" ? "#6b7280" : (signalType === "SELL" ? "blue" : "#dc2626") }}>{displayN}</div>
          </div>
          <div className="action-col">
            <div className="act-label">빅테크</div>
            <div className="act-val" style={{ color: signalType === "HOLD" ? "#6b7280" : (signalType === "SELL" ? "blue" : "#dc2626") }}>{displayB}</div>
          </div>
        </div>
      </div>
      ...
      <style jsx>{`
        ...
        @media (max-width: 480px) {
          ...
          .t-row { grid-template-columns: 90px 1fr; padding: 8px 0; }
