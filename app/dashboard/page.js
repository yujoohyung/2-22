...
export default function DashboardPage() {
  ...
  // [최적화] 전역 스토어에서 실시간 데이터 가져오기 (로딩 X)
  const nowQuote = marketData[CODE] || { price: 0, high: 0 };
  const otherPrice = marketData["465610"]?.price || 0; // 빅테크 가격
  ...
  // [최적화] 차트 데이터: 캐시가 있으면 그것을 쓰고, 없으면 빈 배열로 시작
  ...
  /** 일자별 시세 로드 (캐시 사용으로 멈춤 현상 해결) */
  useEffect(() => {
    // 이미 캐시에 데이터가 있으면 API 호출 안 함 (빠른 이동 시 멈춤 방지)
    ...
    (async () => {
      try {
        ...
        setApiRows(resRows);
        setDailyCache(CODE, resRows); // [중요] 캐시에 저장
      } catch (e) {
        console.error(e);
      }
    })();
